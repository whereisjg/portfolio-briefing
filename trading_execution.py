...
def fetch_kis_daily_closes(code, context, lookback_days=400, chunk_days=120):
    ...
    for start, end in historical_date_ranges(
        now, lookback_days=int(lookback_days), chunk_days=int(chunk_days)
    ):
...
def fetch_kis_index_daily_closes(symbol, context, lookback_days=400, chunk_days=120):
