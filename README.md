...
KoAct 미국나스닥성장기업액티브(`0015B0`)의 완료된 일봉으로 20일·60일 이동평균을 계산합니다. 최근 3거래일 신호가 모두 같을 때만 비중을 변경하며, 엇갈리면 중립 비중을 유지합니다.

| ETF | 위험 선호 | 중립 | 위험 회피 |
| --- | ---: | ---: | ---: |
| KODEX 미국S&P500데일리커버드콜OTM | 55% | 50% | 40% |
| KoAct 미국나스닥성장기업액티브 | 30% | 25% | 15% |
| KODEX 미국머니마켓액티브 | 15% | 25% | 45% |
...
- 매수는 KIS 주문가능금액 범위에서만 계산합니다. 매수·매도를 합친 일일 총 매매금액은 계좌 총자산의 2%를 넘지 않습니다. 1주를 살 수 없는 잔액은 다음 영업일로 이월합니다.
- 위 2% 일일 한도는 실계좌에만 적용합니다. 모의투자는 당일 한도와 누적 체결액 차감을 적용하지 않되, 한 번의 workflow에서 최대 10만원(`paper_test_order_limit_krw`)까지만 주문해 반복 검증할 수 있게 합니다.
