...
- cron-job.org가 영업일 10:00 KST에 ISA 실주문 workflow를 실행하면 잔고·예수금·가격을 조회합니다. 실행이 실패하거나 누락되면 GitHub Actions 백업 workflow가 10:12 KST에 한 번만 재실행합니다. `paper` 또는 주문 없는 dry-run 성공은 백업 실행 완료로 간주하지 않습니다.
