...
완료된 일봉으로 KoAct·나스닥 커버드콜·S&P 커버드콜·환노출형 TIGER 일본니케이225로 구성한 계좌 위험자산 묶음(50%), 나스닥100(25%), S&P500(25%)의 HMA(20일·40일)를 계산합니다. 각 신호의 최근 20개 상태에서 공통으로 완료된 마지막 3거래일의 복합 점수가 같을 때만 비중을 변경하므로, 국내·미국 휴장일 차이로 주문이 불필요하게 중단되지 않습니다. 각 신호의 종가가 HMA200 아래라면 위험 선호 전환은 중립으로 제한합니다.
...
- cron-job.org가 영업일 10:00 KST에 ISA 실주문 workflow를 실행하면 잔고·예수금·가격을 조회합니다. 주문 전 조회가 일시적으로 실패하면 5분 뒤 한 번만 자동 복구를 시도합니다. GitHub Actions 백업 workflow는 cron 실행 자체가 누락된 경우에만 10:12 KST에 한 번 실행합니다. `paper` 또는 주문 없는 dry-run은 백업 실행으로 간주하지 않습니다.
- 계좌 위험자산 묶음(50%, KoAct·나스닥 커버드콜·S&P 커버드콜·환노출형 TIGER 일본니케이225의 중립 목표비중 가중), 나스닥100(25%), S&P500(25%)의 완료된 일봉에서 HMA(20일·40일)를 계산합니다. 각 신호의 최근 20개 상태에서 공통으로 완료된 마지막 3거래일의 복합 신호가 같을 때 목표 비중을 변경합니다. 종가가 HMA200 아래면 위험 선호 전환은 중립으로 제한합니다. 상승 추세는 위험 선호, 하락 추세는 위험 회피, 그 외에는 중립 비중을 적용합니다.
...
- 같은 날 이미 체결된 대상 ETF 주문은 일일 총 매매 한도에서 차감합니다. 1차 주문의 모든 접수번호가 조회되고 미체결분이 취소 또는 전량 체결된 것이 확인될 때만 2차 주문을 전송합니다. 상태가 불명확하면 2차 주문을 보류합니다. GitHub Actions concurrency도 동시에 두 실행이 겹치지 않게 합니다.
- 주문 전송 중 오류가 나면 이후 주문과 2차 주문을 중단합니다. 앞선 주문의 체결·취소 결과와 오류 사유는 브리핑에 남기고, workflow는 잔고 브리핑까지 계속 진행합니다.
- 주문을 보내기 전의 잔고·호가·추세 조회 오류는 5분 뒤 한 번 자동 재시도합니다. 주문 접수 뒤 오류 또는 접수 상태가 불명확한 경우에는 중복 주문을 막기 위해 재주문하지 않고, 주문내역·잔고 브리핑으로 상태만 확인합니다.
